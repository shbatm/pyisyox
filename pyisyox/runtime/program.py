...
class _ProgramBase:
    ...
    async def run(self) -> None:
        ...
        """
        await self._client.run_program_command(self._record.address, "run")

    async def stop(self) -> None:
        """Stop a running program / folder."""
        await self._client.run_program_command(self._record.address, "stop")

    async def enable(self) -> None:
        """Enable the program / folder."""
        await self._client.run_program_command(self._record.address, "enable")
    ...
    async def disable(self) -> None:
        ...
        """
        await self._client.run_program_command(self._record.address, "disable")
...
class Program(_ProgramBase):
    ...
    async def run_then(self) -> None:
        ...
        """
        await self._client.run_program_command(self._record.address, "runThen")

    async def run_else(self) -> None:
        """Run the program's ``else`` clause."""
        await self._client.run_program_command(self._record.address, "runElse")

    async def run_if(self) -> None:
        """Re-evaluate the program's ``if`` condition (without running
        the matching clause's actions)."""
        await self._client.run_program_command(self._record.address, "runIf")

    async def enable_run_at_startup(self) -> None:
        """Mark the program as auto-run on controller boot."""
        await self._client.run_program_command(self._record.address, "enableRunAtStartup")

    async def disable_run_at_startup(self) -> None:
        """Clear the auto-run-on-boot flag."""
        await self._client.run_program_command(self._record.address, "disableRunAtStartup")
