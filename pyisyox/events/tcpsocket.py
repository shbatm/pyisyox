...
from pyisyox.constants import POLL_TIME, RECONNECT_DELAY, EventStreamStatus
from pyisyox.events import strings
from pyisyox.events.eventreader import ISYEventReader
from pyisyox.events.router import EventRouter
...
_LOGGER = logging.getLogger(__name__)  # Allows targeting pyisyox.events in handlers.
...
class EventStream:
    ...
    def _lost_connection(self, delay: int = 0) -> None:
        ...
        _LOGGER.warning("PyISYoX lost connection to the ISY event stream.")
    ...
    def watch(self) -> None:
        ...
        if not self._subscribed:
            _LOGGER.debug("PyISYoX watch called without a subscription.")
            return
        ...
        while self._running and self._subscribed:
            ...
            try:
                ...
                _LOGGER.warning(
                    "PyISYoX encountered an error while reading the event stream: %s.",
                    ex,
                )
                self._lost_connection()
                return
                ...
                _LOGGER.warning(
                    "PyISYoX encountered a socket error while reading the event stream: %s.",
                    ex,
                )
                self._lost_connection()
                return
            ...
            for message in events:
                try:
                    ...
                    _LOGGER.warning(
                        "PyISYoX encountered while routing message '%s': %s",
                        message,
                        ex,
                    )
                    raise
