...
from pyiox.connection import ISYConnectionInfo
from pyiox.constants import POLL_TIME, RECONNECT_DELAY, EventStreamStatus
from pyiox.events import strings
from pyiox.events.eventreader import ISYEventReader
from pyiox.events.router import EventRouter
from pyiox.exceptions import ISYInvalidAuthError, ISYMaxConnections, ISYStreamDataError

if TYPE_CHECKING:
    from pyiox.isy import ISY

SOCKET_HEARTBEAT = 30
SOCKET_HB_GRACE = 5

_LOGGER = logging.getLogger(__name__)  # Allows targeting pyiox.events in handlers.
...
class EventStream:
    ...
    def connect(self) -> bool:
        ...
        if not self._connected:
            try:
                ...
                _LOGGER.exception(
                    "PyIoX could not connect to ISY event stream. %s", err
                )
    ...
    def unsubscribe(self) -> None:
        ...
        if self._subscribed and self._connected:
            ...
            try:
                ...
                _LOGGER.error(
                    "PyIoX encountered a socket error while writing unsubscribe message to the socket: %s.",
                    ex,
                )
            self._subscribed = False
            self.disconnect()
    ...
    def _lost_connection(self, delay: int = 0) -> None:
        ...
        _LOGGER.warning("PyIoX lost connection to the ISY event stream.")
    ...
    def watch(self) -> None:
        ...
        if not self._subscribed:
            _LOGGER.debug("PyIoX watch called without a subscription.")
            return
        ...
        while self._running and self._subscribed:
            ...
            try:
                ...
                _LOGGER.error(
                    "PyIoX reached maximum connections, delaying reconnect attempt by %s seconds.",
                    RECONNECT_DELAY,
                )
                ...
                _LOGGER.warning(
                    "PyIoX encountered an error while reading the event stream: %s.", ex
                )
                self._lost_connection()
                return
            except OSError as ex:
                _LOGGER.warning(
                    "PyIoX encountered a socket error while reading the event stream: %s.",
                    ex,
                )
                self._lost_connection()
                return
            ...
            for message in events:
                try:
                    ...
                    _LOGGER.warning(
                        "PyIoX encountered while routing message '%s': %s", message, ex
                    )
                    raise
