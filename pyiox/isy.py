...
from pyiox.clock import Clock
from pyiox.configuration import ConfigurationData
from pyiox.connection import Connection, ISYConnectionInfo
from pyiox.constants import (
    CMD_X10,
    URL_QUERY,
    X10_COMMANDS,
    EventStreamStatus,
...
from pyiox.events.tcpsocket import EventStream
from pyiox.events.websocket import WebSocketClient
from pyiox.exceptions import ISYConnectionError, ISYNotInitializedError
from pyiox.helpers.events import EventEmitter
from pyiox.logging import _LOGGER, enable_logging
from pyiox.networking import NetworkResources
from pyiox.node_servers import NodeServers
from pyiox.nodes import Nodes
from pyiox.programs import Programs
from pyiox.variables import Variables
...
class ISY:
    ...
    def _auto_reconnecter(self) -> None:
        ...
        while self.auto_reconnect and not self.auto_update:
            _LOGGER.warning("PyIoX attempting stream reconnect.")
            self._events = None
            self._events = EventStream(
                self, self.conn.connection_info, self._on_lost_event_stream
            )
        ...
        if not self.auto_update:
            self._events = None
            _LOGGER.warning("PyIoX could not reconnect to the event stream.")
            self.connection_events.notify(EventStreamStatus.RECONNECT_FAILED)
        else:
            _LOGGER.warning("PyIoX reconnected to the event stream.")
